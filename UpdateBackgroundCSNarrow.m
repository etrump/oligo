function [TotalCS] = UpdateBackgroundCSNarrow(TotalMass, Dpg0, rho_T, time)
%TotalMass in ug/m3
...
Dp_0 = 10;
Dp_f = Dpg+20; %NOTE THIS COULD AUSE ERROR IF IT IS CUTTING OFF PART OF BACKGROUND DISTRIBUTION - ONLY FOR SMALL DEVIATIONS

logSigma = 0.23;  % choose a standard deviation for the distribution (see S&P tbl 8.3 pg371 for typical values)
Sigma = exp(logSigma);
...
logDp = logDp_0:0.1:logDp_f;
 ...
 Ceq = modelAtm.Pop1.Ceq;
 Aterm = 4*modelAtm.SOA.Diff*modelAtm.SOA.MW*modelAtm.Pop1.DriveForce/modelAtm.SOA.rho;
  for i = 1:length(Dp)-1
    d(i) = TotalMass/((2*pi)^(1/2)*logSigma)*exp(-(logDp(i)-logDpgV)^2/(2*(logSigma)^2));
    Bterm(i) = log((Dp(i)^2-2*Aterm*time)^(1/2));
    d(i) = Dp(i)/(Dp(i)^2-2*Aterm*time)*TotalMass/((2*pi)^(1/2)*logSigma)*exp(-(Bterm(i)-logDpgV)^2/(2*(logSigma)^2));
